...
1. **Which state governs** — new, and created by closing the address item. The CMRA is in **Sioux Falls, South Dakota**; the terms' governing-law clause, the sole proprietorship's home state, the assumed-name filing route and the recommended LLC all say **Tennessee**. Nothing is wrong as published — a contact address need not sit in the governing-law state — but the only address a reader can now see is in a state the documents never mention. Which way it resolves depends on whether Tennessee is still the real base or South Dakota is becoming the domicile, and Sioux Falls is *the* town for the latter. "Which state" in [legal/README.md](legal/README.md) has both branches. Cheap to fix now, awkward after customers have accepted the terms.
...
3. **An LLC** — recommended, not a blocker. There is no liability shield today; see the entity note in [legal/README.md](legal/README.md). **Which state it forms in is item 1**, so settle that first — forming in the wrong one costs a dissolution and a refiling.

**The published postal address is a CMRA: 5013 S Louise Ave, Unit #803, Sioux Falls, SD 57108**, live 4 Aug 2026 in privacy §1 and terms §18. It closed the last blank in the legal documents. The draft banner at `/privacy` and `/terms` **self-disabled** — `unresolved()` in [draft.js](frontend/src/draft.js) reads the document text, so filling the address in removed the banner with no code change, which is exactly what that design was for. `draft.test.js` now asserts *no* blanks remain in either document, so a new `[PLACEHOLDER]` in a published legal page fails CI rather than shipping. No residence appears anywhere on the site. **A CMRA cannot accept service of process** — if an LLC follows, its registered agent covers that, so do not buy a second service before settling item 1.
