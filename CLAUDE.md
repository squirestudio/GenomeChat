...
**The legal documents live in [legal/](legal/)** and are written from an audit of what the code actually does, not from a template. `legal/README.md` is the decision log — read it before changing anything in this area, because several product behaviours exist to satisfy specific clauses.

**They are reviewed and approved as of 4 Aug 2026**, on the founder's confirmation. The "draft pending legal review" line is out of both documents and must not come back; `legal/README.md` is **not published** and must never be linked from either document, since it names the full legal name and the unshipped entity plan, both of which deliberately appear nowhere on the site. **A material edit means moving the `Last updated` and `Effective` dates in the same commit** — terms §17 promises a revised date on material change, so a stale date breaks a commitment the document makes about itself. **There is no draft banner and there should not be one.** `DraftBanner` and `draft.js` are deleted — a component whose job is to announce that these documents are unfinished has nothing true left to say about approved ones. The guard did not go with it: it moved into [legal.test.js](frontend/src/legal.test.js) (was `draft.test.js`), which fails on an unfilled `[PLACEHOLDER]`, on any "draft"/"pending review"/`legal/README` language reaching a published document, and on a missing revised date. **A blank in a published legal document should fail the build rather than be explained to the reader who found it.**
...
**Test-mode prices live under different variable names in production, and a missing one fails silently.** Railway runs live keys and reads test-mode prices from `STRIPE_TEST_PRICE_*`, a parallel set to the `STRIPE_PRICE_*` used locally. If one is unset the purchase option simply **does not render** — no error, no log line, just a missing card in the modal. That is how a 200-credit pack existed in live and was invisible in test. `scripts/setup_stripe_test.py` now prints both sets of names for exactly this reason.
...
**A test asserting a pricing constant must import it, not repeat it.** Changing `CREDITS_PER_PACK` from 50 to 200 broke four tests that hardcoded the old number — and they **passed locally and failed in CI**, because the webhook runs in the server process and `docker compose restart` had not been run. Same stale-process trap as the classifier change: after editing anything the running uvicorn imported, restart before believing a green suite.
...
### Projects — a query belongs to many, and the migration that got it there

**Attribution was fixed at ask-time and invisible.** `project_id` rode on the chat request and landed on the row, so queries were always filed correctly — but `/projects/queries/recent` ignored it, so the sidebar list was identical whichever project was selected. The feature was ~80% built and 0% visible. Now the list scopes, the heading names the project, an empty project says so rather than vanishing, and the input reads "Saving to *project*".

**`queries.project_id` is gone. Membership lives in `query_projects`.** A single nullable FK meant one query in one project, and research does not divide that way — BRCA1 belongs under "breast cancer" and "DNA repair" at once. A Duplicate button was considered and rejected: it produces two rows that diverge the moment either is touched, two entries in history with nothing to tell them apart, and two share tokens for "the same" query.

**`ON DELETE CASCADE` on both FKs is load-bearing, not tidiness.** Account deletion removes queries with a bulk `.delete(synchronize_session=False)`, which never loads rows and so never fires an ORM cascade. Without the database enforcing it, account erasure fails on a foreign-key violation — a published data-protection commitment broken by two missing words. `test_privacy_rights` covers exactly this by filing a query before deleting the account.

**The migration is expand → verify → contract, and the verify is the point.** `_migrate_to_many_projects()` in [models.py](genomics_backend/database/models.py) is deliberately *not* in the `_run_migrations()` list, because every statement there is individually try/excepted: a backfill that failed silently followed by a drop that succeeded would destroy every association in the database, leaving one debug line behind. The drop is guarded by a count — if links do not match rows carrying a `project_id`, the column stays and the next boot retries. Losing the column is unrecoverable; leaving it one deploy longer costs nothing. Verified against a rewound database: 2 links created, column dropped, unfiled rows untouched, and a second boot is a no-op.

**Order matters in the model file.** `query_projects` must be defined *above* `class Project`, which references it in a relationship evaluated at class-definition time. Putting it above `class Query` — where it reads more naturally — crash-loops the app with `NameError: name 'query_projects' is not defined`.

**`PATCH /projects/queries/assign`** toggles membership: `{query_ids, project_id, member}`, with `project_id: null` meaning "remove from every project". Adding twice is not an error and removing something absent is not either, because the UI toggles and a toggle that fails on a double-click is a worse UI. **Both sides are owner-checked** — the queries through `_owned_by`, and the destination project separately, since a valid id of *someone else's* project would otherwise file your rows into it. `DELETE /{project_id}/queries/{query_id}` now unfiles rather than deletes; deleting for real is `DELETE /queries/{id}`.

**Filtering happens in `/queries/recent`, not `GET /projects/{id}`**: that route returns `ProjectWithQueries` carrying the full `results` payload of every query, and the sidebar needs one line each. It joins `query_projects` rather than using `.any()` because the link table is indexed on `project_id` and this is the sidebar's hot path.
...
"Get up to 150 free credits." `REFERRAL_CREDITS` (50) lands on the referrer when someone joins through their link, capped at `REFERRAL_CAP` (3). Measured rather than guessed: a credit is $0.0128 of model tokens, so the cap costs at most **$1.92** per account. Abuse was expected from the start; the cap is the cost control, not a deterrent.

**Credit happens inside account creation, and that is the whole design.** `credit_referral` runs in the OAuth callback: the code arrives with the sign-in, the referrer's counter goes up, the code is discarded. **No column anywhere records who introduced whom.**

The first version credited on the referee's *first question*, which meant parking the referrer's code on their row until then via a `pending_referral_code` column. That was defensible — it erased itself on conversion, and it made farming require a real query — but a few minutes is long enough to reach a backup, and a record of who introduced whom is a map of who knows whom. In a genomics product that is a real inference, since people refer family and genetics is familial. Crediting at signup removes the window entirely; the column is dropped by migration. **What was traded away:** the referee no longer has to *use* the product, only register. The cap absorbs that.

**Two guards in `credit_referral`**: an established account cannot claim a code (`total_queries` must be zero), and nobody can refer themselves. A test walks every column on a referee's row and asserts none of them holds the referrer's code.

**The code travels through OAuth `state`**, the parameter that exists to survive that round trip. An endpoint to claim a code afterwards would let an established account backdate one. On the browser side `captureReferral()` puts an arriving `?r=` in `localStorage` and **sends nothing** — so "signed-out visitors are never recorded" holds without a special case, and a click that never becomes an account leaves no trace.
...
**The findings are no longer research-only, and the line moved for a good reason.** They are computed from named sources and link the records behind them — the same contract as every other panel — and they make the underlying research *easier* to reach rather than harder. Someone reading about their own condition has as much business knowing that curators disagree as a researcher does; hiding it would be paternalistic, and this product's argument is that uncertainty gets shown rather than smoothed over. **The sharp line is now: computed findings are for everyone, generated speculation is not.** That is what research mode is for.

**`/research/evidence` reads the papers, and it is the first place this product does.** Everything before it referenced literature by identifier — PMID, title, year — which is enough to link and not enough to compare. It fetches the abstracts of what each curator cited and asks the model to say **what each side actually weighed**, which is the difference between "five curators disagree" and "ClinGen cited two family studies with a mechanism; Orphanet cited a clinical description".

**The abstracts were already arriving and being discarded.** `research_topics` makes the identical `efetch?db=pubmed&retmode=xml` call and regexes out only `<DescriptorName>`; the same response carries `<AbstractText>`. No new source, no new integration — a different parse of a call already being made.

Two things `fetch_pubmed_abstracts` handles that a naive version does not. **`<PubmedBookArticle>` as well as `<PubmedArticle>`** — curators cite GeneReviews constantly and PubMed returns those as books, so matching one wrapper silently drops real evidence (Orphanet's Caffey disease citation was exactly that). And **structured abstracts keep their labels**, because "METHODS: 12 families" and "RESULTS: 12 families" are different claims.

**The prompt is grounded hard and refuses to adjudicate.** It may use only the supplied abstracts, must say when a paper was cited without one, and must not decide which curator is right — that is a curation judgement the model has no standing to make. A confident sentence about a cohort size the abstract never stated is the failure that costs a researcher a month.

**It is an explicit action because it is the only thing on the panel that costs a model call.** Everything else is computed and free. Not charged a credit yet — that waits on a token measurement rather than a guess.

**Citations are resolved to titles.** A bare PMID is a receipt — "15864348" tells a reader nothing about which side of a disagreement read what, which is the whole point of listing them. `fetch_pubmed_titles` batches the lookup in one `esummary` call across every finding, and an unresolved title falls back to the number so the link never disappears. **That function already existed** and was written for exactly this; a second copy got added before anyone checked, and Python silently used the older one. Grep before adding a fetcher.
...
**Findings carry their evidence, and the panel renders it.** "Curators disagree" as a bare headline was the first real complaint about research mode — it tells a researcher a disagreement exists and nothing they can do with it. Each disputed pair now shows **who classified it as what, when**, and links every PMID any curator cited. All of that was already fetched from GenCC and discarded at the panel. The two ends of a disagreement have usually read different evidence, and naming it is what turns a headline into a lead.

**Measured on real genes before believing it.** RYR1 8 findings, SCN1A 7, COL1A1 6, BRCA1 3 — and **CYP2C19 just 1**, because pharmacogenes have `gencc=0, clingen=0, prevalence=0` and a LOEUF of 1.79, so three of five analyses have nothing to work with and the constraint check cannot fire. **Pharmacogenes are the blind spot**; they would need their own analysis over ClinPGx star alleles and CPIC guidance. Do not conclude the analyses are weak from a CYP gene.
...
**The Settings toggle is `MyDNA` / `Research (Beta)` and is visible to everyone.** Selecting Research opens one modal that does explanation, consent and unlock together. Hiding the toggle from locked accounts was tried first and is worse: a locked door with no sign on it tells a curious researcher nothing, and the modal has to explain the mode anyway. `research_unlocked` on `/auth/me` decides whether that modal shows a code field or an "I understand" button.

**That modal is consent rather than decoration**, and it gates the first switch into the mode. Every other screen trains the reader to expect conservative, sourced, hedged answers — research mode is the one place that changes, and someone arriving from the personal side carrying those assumptions would be badly served by a footnote. It states the distinction the whole design rests on: **the findings are computed and checkable; the reasoning about them is generated, reviewed by nobody, and can be confidently wrong.** Acknowledged once per account, because a wall that appears every time is one people learn to click through without reading.

**Findings are fetched after the answer lands, never folded into the stream.** The endpoint runs no model and costs nothing, and keeping it off the streaming path means a failure there can never damage an answer the reader already paid for. `ResearchFindings` renders under each gene answer when the mode is on.

**`skipped` is rendered, not hidden.** An analysis whose inputs were missing is not an analysis that found nothing, and collapsing the two would let a clean panel read as a clean bill of health on checks that never ran — the exact mistake the upstream-drift audit exists to prevent.

**Still to build:** (open-ended, generated, cross-source outputs that are *not* medical advice and must be verified before acting), the frontend mode toggle, and the chart grammar. The long-term shape the founder has in mind is **MyLab.chat** — researchers uploading their own work and notes to cross-reference against the public corpus.
...
**Sharing was removed on 8 Aug 2026 and should not come back without the two things it lacked.** `POST /queries/{id}/share` minted a token and `GET /share/{token}` served the whole stored answer to anyone holding it, unauthenticated. **There was no revocation** — the token was set once and never cleared, so a click was permanent and irreversible — and **it could carry more than the reader expected**: variants are never stored, but the model's prose is, and with a DNA file loaded that prose routinely names a genotype. The question text went too, and the question is often the revealing part. The 404 also claimed links could expire, and none did.

Removing the routes revoked every link ever minted, and a migration nulls `queries.share_token` so the table is not holding secrets for a feature that no longer exists. The column stays, so reinstating this is small — but it needs an un-share endpoint, a list of active links in Settings, and a warning before minting. `test_access_control.py` asserts both routes 404 and that no row retains a token, because putting it back is otherwise silent. `share_router` was renamed `queries_router`, which is what it always was once sharing left it.
