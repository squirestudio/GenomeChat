# Terms of Use

**Draft pending legal review. See `legal/README.md` for what remains open.**

**Last updated:** 30 July 2026
**Effective:** 30 July 2026

---

## 1. Agreement

These terms are between you and **Red Wolf Agency, a sole proprietorship of
Benjamin Kenneth Brown** ("MyDNA", "we"). By using mydna.chat you accept
them. If you do not, please do not use the service.

## 2. What MyDNA is

MyDNA is a research and educational tool. It queries public biomedical databases
and uses an AI model to explain what they contain, in plain language, with the
sources shown.

## 3. Not medical advice

**This is the most important term here.**

MyDNA does **not** provide medical advice, diagnosis or treatment. It is not a
medical device, a clinical decision support tool, or a substitute for a
qualified professional. Nothing it produces is a diagnosis, prognosis, risk
assessment, or recommendation about your health.

**Speak to your clinician about your personal situation.** Do not start, stop
or change any treatment, test or medication because of anything you read here.
In an emergency, contact emergency services.

Where MyDNA names a clinical genetic test that exists for a gene, that is
**information from a public registry, not a recommendation** that you take it.
We have no relationship with any testing laboratory and receive nothing if you
contact one.

Genetic information is difficult to interpret correctly, and carrying a variant
is not the same as having or developing a condition. Interpretation depends on
family history, clinical context and other factors that a raw data file cannot
supply. A qualified genetic counsellor or clinician is the right person to help
with that.

## 4. Accuracy, and its limits

MyDNA draws on public databases maintained by others. We do not control their
accuracy, completeness or availability, and their contents change.

Answers are written by a large language model. **It can be wrong.** It can
misread a source, overstate a finding, or omit relevant context. Every answer
names the sources it used and links to the original records specifically so you
can check it, and you should treat that as part of using the tool rather than an
optional extra.

MyDNA is provided on an "as is" and "as available" basis, with no warranty of
accuracy, fitness for a particular purpose, or uninterrupted availability.

## 5. Eligibility and where MyDNA is offered

You must be at least **18** and able to form a binding contract. Do not use
MyDNA on behalf of another person without their informed consent, and do not
upload another person's genetic data without it.

**MyDNA is operated from the United States and is offered to users in the
United States.** It is not directed at, marketed to, or intended for
individuals in the European Economic Area or the United Kingdom. Pricing is in
US dollars, the service is offered only in English, and we do not target those
regions.

The site is reachable from anywhere, and we are not going to pretend otherwise.
If you choose to use MyDNA from outside the United States, you do so on your
own initiative and are responsible for complying with your local law.

We honour the data rights described in the [Privacy Policy](./privacy-policy.md)
— access, correction, deletion, portability — **for everyone who asks,
regardless of where they live**. That is a deliberate choice rather than an
obligation we have accepted.

## 6. Your data and your file

You keep all rights in anything you upload or ask. By using MyDNA you give us
only the permission needed to run the service: to process your question and any
relevant variants in order to answer it.

How genetic data is handled — parsed in your browser, never uploaded or stored —
is set out in the [Privacy Policy](./privacy-policy.md). We do not use your data
to train any model.

## 7. Acceptable use

Do not:

- Use MyDNA to make clinical decisions, or present its output as medical advice
  to anyone else
- Upload another person's genetic data without their informed consent
- Attempt to re-identify any individual from any data
- Use it as a bulk proxy for the underlying databases, or scrape it — these are
  shared public resources with their own rate limits, and abusing them through
  us harms every other user
- Circumvent rate limits, quotas or access controls, or use automated means to
  create accounts or queries at scale
- Attempt to extract, resell or redistribute the aggregated data
- Interfere with the service or attempt to gain unauthorised access
- Use it unlawfully, or in breach of the terms of the underlying data sources

We may apply rate limits, and may suspend access that damages the service or
the sources it depends on.

## 8. Third-party data and attribution

The scientific content comes from third parties, each with its own terms and
citation requirements — including NCBI (ClinVar, dbSNP, dbVar, MedGen, GTR,
PubMed, PMC, GDC), EMBL-EBI (Ensembl, GWAS Catalog), the Broad Institute
(gnomAD), UniProt, AlphaFold, ClinGen, HPO, the Monarch Initiative,
Reactome, GTEx, STRING, Open Targets and ClinPGx.

MyDNA claims no ownership of that content and displays it with attribution. Your
use of it is subject to those sources' terms, which you are responsible for
observing if you reuse anything you find here.

Sources whose terms restrict commercial use are not used. OMIM was
disconnected for that reason rather than licensed. See
`legal/data-source-licensing.md`.

## 9. Accounts

You are responsible for activity under your account. Sign-in is via Google; keep
that account secure. Tell us promptly if you believe your account has been
misused.

## 10. Paid plans

- **Free allowance.** A number of queries at no cost, currently 20 for signed-in
  users. We may change this.
- **Subscription.** Billed monthly through Stripe, renewing until cancelled.
  Cancel any time through the customer portal in the app; access continues to
  the end of the paid period.
- **Credits.** One-time purchases. They do not expire but are not refundable
  once used.
- **Bring your own key.** A one-time purchase of the right to store your own
  Anthropic API key. You then pay Anthropic directly for usage.
- **Prices** are shown before purchase and may change with notice. Changes do not
  affect a period already paid for.
- **Refunds.** Within **14 days**: unused credits are refundable, and a
  subscription can be refunded for the current period if you have not used it
  meaningfully. After 14 days, refunds are at our discretion — ask, and we will
  be reasonable. Your statutory rights are unaffected either way.
- **Taxes** may apply depending on where you are.

## 11. Availability

MyDNA is an independent project, not an enterprise service. It may be
unavailable, and features may change or be withdrawn. Nothing here is a
guarantee of continued availability. If it is ever discontinued we will give
reasonable notice and an opportunity to export your data.

## 12. Limitation of liability

To the fullest extent the law allows, MyDNA is not liable for any indirect,
incidental, special or consequential loss, or for any loss arising from a
decision made on the basis of its output. **Health decisions belong with your
clinician**, and this term reflects that rather than trying to escape it.

Where liability cannot be excluded, it is limited to the total amount you paid
us in the **six months** before the claim arose.

Nothing here excludes liability for death or personal injury caused by
negligence, fraud, or anything else that cannot lawfully be excluded — and if
you are a consumer, your statutory rights stand regardless of this section.

## 13. Indemnity

You agree to indemnify us against claims arising from your misuse of MyDNA, or
from uploading another person's genetic data without their consent.

## 14. Termination

You may stop using MyDNA and delete your account at any time. We may suspend or
terminate access for breach of these terms, or where continued access would harm
the service or its data sources. Sections 3, 4, 6, 12 and 13 survive.

## 15. Governing law

These terms are governed by the laws of **the State of Tennessee, United States**, and disputes are
subject to the courts of **the State of Tennessee, United States**. If you are a consumer, this does
not deprive you of the protection of your local law.

## 16. Changes

We may update these terms. Material changes will be noted on the site with a
revised date. Continuing to use MyDNA after that means you accept them.

## 17. Contact

**info@redwolfagency.co**
